...
public class BlackjackGame {
    ...
    public int score(CardPile p) {
      //FIX THIS
      int score = 0;
      for(int i=0; i < p.getCards().size(); i++) { //p object gets the AL from Cardpile class and its size
        score += p.getCards().get(i).getRank(); //score is the sum of the cards rank
        }
        return score;
        /*
      int score =0; //first attempt
        for (int i = 0; i < p.size(); i++) {
        score += p.get(i).getRank();
      } */
    }
